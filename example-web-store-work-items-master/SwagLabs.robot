...
${FAIL}=                    ${False}
${OUTPUTDIR}=               C:/Users/Jan/Desktop/moje/MyRobocorp/example-web-store-work-items-master/screenshots
...
Process order
    ...
    ${now} =    Replace String    ${now}    .    _
    Capture page screenshot    ${OUTPUTDIR}/cart_${now}.png
    Checkout    ${name}    ${zip}

Reset application state
    Click Button    css:.bm-burger-button button
    Click Element When Visible    id:reset_sidebar_link
    Induce random error    0.8    Application cannot be reset
...
Assert cart is empty
    #${tt} =    Get Text    css:.shopping_cart_link
    #Log    ${tt}
